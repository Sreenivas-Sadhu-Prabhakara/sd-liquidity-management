...
```bash
mvn -B verify
docker build -t bian/sd-liquidity-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-liquidity-management ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-liquidity-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
